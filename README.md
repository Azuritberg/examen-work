...
En experimentell pipeline som:

1. Lyssnar på en webbradio
2. Transkriberar ljudet med OpenAI (Speech-to-Text)
3. Låter en LLM tolka innehållet
4. Skriver ut resultatet på en kvittoskrivare (Star TSP100III)

Systemet körs lokalt på macOS.
...
Webbradio (stream URL)  
↓  
ffmpeg spelar in små ljudbitar (t.ex. 12 sekunder) → sparas som `.wav`  
↓  
Transkribering via OpenAI (STT) → text  
↓  
LLM tar text + minne → avgör om något ska skrivas ut  
↓  
Star TSP100III får rå ESC/POS-data via CUPS (`lp -o raw`)

---

# Installation (macOS – nybörjarvänligt)

## 1. Installera Homebrew (om du inte har det)
...
Gå till projektmapp => finns på GitHub examen-work


Skapa en virtuell miljö
  Detta gör att vi slipper installera paket globalt på datorn.
...
I terminalen:
  ...
  Exempel:  printer Star_TSP100III is idle ...
...
echo "Hej" | lp -d Star_TSP100III

Om detta => Fungerar detta är skrivaren korrekt kopplad.
