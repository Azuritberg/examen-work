...
from intro_text import INTRO_TEXT, INTRO_LOGO, INTRO_DELAY_SECONDS
...
def main() -> None:
    ...
    # # whitespace före loggan så den hamnar ungefär i mitten av kvittot

    # logo_line = center_text(INTRO_LOGO, RECEIPT_WIDTH)

    # intro_block = f"""{logo_line}
    ...
    # linje överst
    if BLOCK_TOP_BORDER:
        intro_lines.append(BLOCK_BORDER_CHAR * BLOCK_BORDER_WIDTH)

    # extra luft före loggan
    intro_lines.append("")
    intro_lines.append("")

    #intro_lines.append(center_text(INTRO_LOGO, RECEIPT_WIDTH))

    intro_lines.append(center_text("SMART RADIO", RECEIPT_WIDTH))
    intro_lines.append(center_text("~ ~ ~ ~ ~ ~ ~", RECEIPT_WIDTH))


    # luft mellan logga och text
    intro_lines.append("")
    intro_lines.append("")
    intro_lines.append("")

    # wrappa själva introtexten normalt
    intro_lines.extend(wrap_text_to_lines(INTRO_TEXT, RECEIPT_WIDTH))

    # extra luft under texten
    intro_lines.append("")
    intro_lines.append("")
    intro_lines.append("")
    intro_lines.append("")
    ...
    # extra whitespace efter hela blocket
    intro_lines.extend([""] * BLOCK_FEED_LINES)

    if DRY_RUN:
        simulate_printer_output_block(intro_lines)
    else:
        if USE_PDF_PRINTING:
            print_lines_as_pdf(
                lines=intro_lines,
            ...
            send_block_to_printer(intro_lines, PRINTER_NAME)
